...
Q = np.dstack(([qx.tolist() + [input_parameters.Qh] for qx in Qx],
...
detuning_y=[i for i in np.where(detuning[:,1]==2)[0]
            if i in np.where(detuning[:,0]==0)[0]][0]
detuning_x=[i for i in np.where(detuning[:,0]==2)[0]
            if i in np.where(detuning[:,1]==0)[0]][0]
